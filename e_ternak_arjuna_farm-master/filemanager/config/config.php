...
$config = array(
	...
	|
	*/
	'base_url' => 'http://localhost/cbt2/',
	// 'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']),
	...
	*/
	...
	'access_keys' => array('myBramantoKey'),
	...
	*******************/
	...
	'fixed_path_from_filemanager'             => array( '../270x135/', '../340x195/' ), //fixed path of the image folder from the current position on upload folder
	...
	'fixed_image_creation_width'              => array( 270, 340 ), //width of image (you can leave empty if you set height)
	'fixed_image_creation_height'             => array( 135, 195 ), //height of image (you can leave empty if you set width)
	/*
	#             $option:     0 / exact = defined size;
	#                          1 / portrait = keep aspect set height;
	#                          2 / landscape = keep aspect set width;
